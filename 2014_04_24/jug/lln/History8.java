...
public class History8 {
	...
	public History8(String s, List<Quote> quotes) {
		this.symbol = s;
		this.quotes = quotes;
	}
	...
	public static History fromFile(String symbol, String path) throws IOException, NumberFormatException, ParseException {
		...
		for (String line : lines) {
			...
			quotes.add(new Quote(DateTimeFormatter.BASIC_ISO_DATE.parse(x[0]), Double.parseDouble(x[1])));					
		}		
	...
	public String toCSV() {		
		...
		for (Quote q: quotes) {			
			sb.append(DateTimeFormatter.BASIC_ISO_DATE.format(q.getDate())).append(",").append(q.getClosePrice()).append("\n");
		}
		return sb.toString();
	}
